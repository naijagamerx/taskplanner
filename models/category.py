...
class Category:
    ...
    def get_by_name(cls, name: str, user_id: int = 1) -> Optional['Category']:
        ...
        try:
            ...
            result = db_manager.fetch_one(query, (user_id, name))
...
class Priority:
    ...
    def save(self) -> bool:
        ...
        try:
            if self.id:
                ...
                query = """UPDATE priority_levels
                          SET name = %s, level = %s, color = %s, description = %s
                          WHERE id = %s"""
                params = (self.name, self.level, self.color, self.description, self.id)
                return db_manager.execute_query(query, params) is not None
            else:
                # Insert new priority
                query = """INSERT INTO priority_levels (name, level, color, description)
                          VALUES (%s, %s, %s, %s)"""
                params = (self.name, self.level, self.color, self.description)
                result = db_manager.execute_query(query, params)
                if result:
                    self.id = result
                    return True
            ...
            print(f"Error saving priority: {e}")
            return False
    ...
    def get_by_id(cls, priority_id: int) -> Optional['Priority']:
        ...
        try:
            ...
            result = db_manager.fetch_one(query, (priority_id,))
